...
public class WelcomePanel extends JPanel implements ChangeListener
{
	...
	private void updateStats()
	{
		...
		int c = (int)carSystem.getStatistics(CarSalesSystem.CARS_COUNT);
		int manufact = (int)carSystem.getStatistics(CarSalesSystem.MANUFACTURERS_COUNT);
		double avgPrice = Math.floor(carSystem.getStatistics(CarSalesSystem.AVERAGE_DISTANCE) * 10 + 0.75) / 10;
		double avgKm = Math.floor(carSystem.getStatistics(CarSalesSystem.AVERAGE_PRICE) * 10 - 0.5) / 10;
		...
		carsLabel.setText("Total number of cars: " + String.valueOf(c));
		manufacturersLabel.setText("Total number of manufacturers: " + String.valueOf(manufact));
