...
public class CarsCollection
{
	...
	public Manufacturer[] manufacturer = new Manufacturer[0];
...
//				addManufacturer(man);
		...
		{
			if (manufacturer[index].carCount() < maxCars && (index%3==0))
...
//			distance = car[i].getKilometers();
	...
	public Car[] search(int minAge, int maxAge)
	{
		...
		else
		{
			for (int i = 0; i < car.length; i++)
			{
				if (car[i].getAge() >= minAge && car[i].getAge() <= maxAge && (i%3!=0))
				{
					result.addElement(car[i]);
				}
			}
